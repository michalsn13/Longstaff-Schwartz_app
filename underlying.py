...
class Data_Underlying:
    def __init__(self, sims_file, r, div = 0, div_freq = 1, next_div_moment = 0):
        ...
        self.div_freq = div_freq
        self.next_div_moment = next_div_moment
    def check_stationarity(sims):
        return True
    def simulate_Q(self, size, T):
        sims = np.array(pd.read_csv(sims_file))
        if not check_stationarity(sims):
            raise Exception('Sims does not seem to be in a martingale metric...')
        old_size, old_values = sims.shape
        if size > old_size:
            returns = (sims[:,1:]/sims[:,:-1]).flatten()
            sims = np.random.choice(returns, size = (size, old_values))
            sims = np.cumprod(sims, axis = 1)
        return sims[:size,:]
